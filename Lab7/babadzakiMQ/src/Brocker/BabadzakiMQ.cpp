...
json BabadzakiMQ::handlePublish(const std::string& client_id, const std::string& queue_name, const std::string& message, QoS qos) {
    ...
    queue->push(msg);
    spdlog::debug("Message published to queue '{}' from client '{}' with QoS={}", queue_name, client_id, static_cast<int>(qos));
...
json BabadzakiMQ::handleSubscribe(const std::string& client_id, const std::string& queue_name, QoS qos, int ack_timeout_sec) {
    ...
    spdlog::info("Client '{}' subscribed to queue '{}' with QoS={}, timeout={}s", 
                 client_id, queue_name, static_cast<int>(qos), ack_timeout_sec);
            ...
            for (auto& [msg_id, pending] : client.pending_acks) {
                ...
                if (elapsed >= pending.timeout_sec) {
                    ...
                    if (queue) {
                        ...
                        spdlog::warn("Message '{}' timed out for client '{}', returned to queue '{}'",
                                   msg_id, client_id, pending.queue_name);
                    }
...
void BabadzakiMQ::loop() {
    ...
    while (running) {
        ...
        try {
            ...
            spdlog::debug("Received request from client '{}': {}", client_id, request_str);
