...
struct Config
{
    ...
    Config(const std::string& name_,
           std::string durability_ = "disk",
           std::string ordering_ = "fifo",
           time_t message_ttl_ = 0)
        : name(name_),
          message_ttl(message_ttl_) {
        std::unordered_map<std::string, Durability> durability_map = {
            {"memory", Durability::Memory},
            {"disk", Durability::Disk}
        };
        std::unordered_map<std::string, Ordering> ordering_map = {
            {"none", Ordering::None},
            {"fifo", Ordering::FIFO}
        };
